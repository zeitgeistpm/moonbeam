...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_initialize(n: BlockNumberFor<T>) -> Weight {
			...
			log::info!(target: "runtime", "on_initialize before if: round: {:?}", round);
			if round.should_update(n) {
				...
				log::info!(target: "runtime", "on_initialize before update: round: {:?}, n: {:?}, current_slot: {:?}", round, n, current_slot);
				// mutate round
				round.update(n, current_slot);
				log::info!(target: "runtime", "on_initialize after update: round: {:?}", round);
	...
	impl<T: Config> Pallet<T> {
		...
		pub(crate) fn prepare_staking_payouts(
			...
			log::info!(target: "runtime", "prepare_staking_payouts: round_info: {:?}", round_info);
